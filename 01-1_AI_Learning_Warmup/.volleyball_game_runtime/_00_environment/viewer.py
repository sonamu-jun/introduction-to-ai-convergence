...
UNLOCK_CODE_LENGTH = 3
UNLOCK_CODE_SHA256 = "c3ea99f86b2f8a74ef4145bb245155ff5f91cd856f287523481c15a1959d5fd1"
TRACKED_INPUT_KEYS = (
    pygame.K_h,
    pygame.K_k,
    pygame.K_u,
...
def load_image(image_key):
    ...
    try:
        ...
        image = _PILImage.open(io.BytesIO(image_bytes)).convert("RGBA")
        return pygame.image.frombuffer(image.tobytes(), image.size, "RGBA").copy()


def matches_unlock_code(value):
    encoded_value = str(value).encode("utf-8")
    return hashlib.sha256(encoded_value).hexdigest() == UNLOCK_CODE_SHA256
...
def load_bnw_config():
    ...
    bnw_mode_enabled = not (
        ...
        and matches_unlock_code(password_value)
    )
...
class Viewer:
    def __init__(self, engine):
        ...
        self.audio_enabled_by_config = (
            not self.bnw_mode and matches_unlock_code(self.bnw_mode_password)
        )
        ...
        self.secret_font = pygame.font.Font(None, 10)
        self.score_text = "0 : 0"
        self.status_text = ""
    ...
    def _maybe_disable_bnw_mode(self, key_value):
        ...
        self.bnw_code_buffer = (
            self.bnw_code_buffer + digit_value
        )[-UNLOCK_CODE_LENGTH:]
        if matches_unlock_code(self.bnw_code_buffer):
